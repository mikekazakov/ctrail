...
{
    CyclicValuesStorage storage({"a", "b", "c"}, 5);
...
TEST_CASE(PREFIX "stores values for a single counter")
{
    ...
    SECTION("Capacity = 1")
    {
        CyclicValuesStorage storage({"a"}, 1);
    ...
    SECTION("Capacity = 2")
    {
        CyclicValuesStorage storage({"a"}, 2);
        REQUIRE(storage.timePointsNumber() == 0);
...
TEST_CASE(PREFIX "stores values for two counters")
{
    ...
    SECTION("Capacity = 1")
    {
        CyclicValuesStorage storage({"a", "b"}, 1);
        REQUIRE(storage.timePointsNumber() == 0);
        auto sink_1 = std::array{i64(0)};
        auto sink_2 = std::array{i64(0), i64(0)};
    ...
    SECTION("Capacity = 2")
    {
        CyclicValuesStorage storage({"a", "b"}, 2);
        REQUIRE(storage.timePointsNumber() == 0);
        auto sink_1 = std::array{i64(0)};
        auto sink_2 = std::array{i64(0), i64(0)};
